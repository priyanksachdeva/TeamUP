...
> Build a production-quality full-stack Team Task Manager web application using the attached PRD. Frontend: React + Tailwind + shadcn/ui. Backend: FastAPI + MongoDB (deviation from PRD's Node/Express, user-approved for Emergent platform). JWT auth + RBAC. Modern dashboard UI, Kanban drag-and-drop board, dark mode, charts, responsive, toast notifications.
